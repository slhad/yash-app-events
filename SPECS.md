...
SPEC-REPLAY-005 | VERIFIED | schema-v1 external packages pin a portable profile and SHA-256 inventory; daemon/CLI evaluation supports full/partial/zone inputs, typed assertions/events, bounded asynchronous progress/status/cancel, structured timeout context, focused impact-reason filters, fail-fast, atomic result output, streamed hashing, metadata/hash-guarded inventory reuse, cold verification, opt-in phase/case/work timings, responsive control RPCs, and stable default JSON; cancellation/progress/hash tests and focused private-suite 10/10 evidence pass (updated 2026-08-18)
